  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3000/api/v1/recipes', axiosConfig);
        if (type === "category" && category) {
          // Filter recipes by category ID
          const filteredRecipes = response.data.recipes.filter(recipe => recipe.category.idCategory === category);
          setRecipes(filteredRecipes);
        } else if (type === "area" && area) {
          const filteredRecipes = response.data.recipes.filter(recipe => recipe.strArea === area);
          setRecipes(filteredRecipes);
        }
        else {
  ...
  return (
    ...
    {recipes.map((recipe) => (
     <Link key={recipe._id} to={`/recipes/${recipe._id}`}>
        <div className="bg-white shadow-lg transition duration-300 ease-in-out transform hover:shadow-xl max-w-sm border border-gray-200  rounded-lg hover:-translate-y-1  overflow-hidden cursor-pointer"> {/* Added cursor-pointer for better UX */}
